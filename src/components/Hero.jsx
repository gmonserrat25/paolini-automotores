  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute left-1/2 top-[15%] w-[75%] max-w-[1073px] -translate-x-1/2 select-none">
        <h2
          className="whitespace-nowrap text-center font-bebas font-bold uppercase leading-[0.82] tracking-[0.02em]"
          style={{
            fontSize: 'clamp(64px, 15.5vw, 230px)',
            backgroundImage:
      ...
      <header className="absolute inset-x-0 top-0 z-20 w-full px-5 py-6 sm:px-8 lg:px-[80px]">
        <nav className="flex items-center justify-between gap-3 sm:gap-6">
          ...
          <ul className="hidden items-center gap-8 lg:flex">
            {NAV_LINKS.map((link) => (
              <li key={link.label}>
                <a
                  href={link.href}
                  className="font-inter text-[15px] text-[#EEEFF2] transition-opacity hover:opacity-70"
          ...
          <div className="flex shrink-0 items-center gap-3 sm:gap-5">
            <a
              href="#contacto"
              className="hidden font-inter text-[15px] text-[#FBFBFD] transition-opacity hover:opacity-70 sm:inline"
              style={{ letterSpacing: '-0.32px' }}
            >
              Financiación
            </a>
            ...
            >
              ...
              <span className="font-inter text-[14px] font-medium text-[#272835] sm:text-[15px]">
                Contacto
              </span>
            </a>
          </div>
      ...
      <div className="absolute inset-x-0 bottom-0 z-20 w-full px-5 pb-10 sm:px-8 lg:px-[80px] lg:pb-14">
        <div className="flex flex-col gap-10 lg:flex-row lg:items-end lg:justify-between lg:gap-12">
          <div className="flex flex-col items-start gap-6 sm:flex-row sm:items-end">
            <p className="max-w-[414px] font-inter text-[20px] leading-[30px] text-white">
          ...
          <h1 className="max-w-[466px] font-bebas text-[38px] leading-none text-white sm:text-[52px] md:text-[60px] lg:text-[64px]">
